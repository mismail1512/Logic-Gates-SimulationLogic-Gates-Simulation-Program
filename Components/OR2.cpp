...
void OR2::setselected(bool state)
{
	selected = state;
}

bool OR2::getselected() const
{
	std::cout << selected;
	return selected;
}

GraphicsInfo * OR2::get_component_PGInfo()
{
	return m_pGfxInfo;
}

void OR2::SaveComponent(ofstream & savefile)
{
	savefile << 6 << endl;// number to indicate which gate we are using 
